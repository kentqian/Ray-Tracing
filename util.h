...
struct Material {
	Material( Colour ambient, Colour diffuse, Colour specular, double exp, double refl_ratio) :
		ambient(ambient), diffuse(diffuse), specular(specular),
		specular_exp(exp),reflect_ratio(refl_ratio) {}
...
struct Ray3D {
	Ray3D() {
		...
		intersection.texture_flag = false;
	}
